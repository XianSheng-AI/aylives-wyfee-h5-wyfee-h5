      ...
      >
        ...
        <div>
          ...
          <p> <img
              src="../../assets/images/decoration/icon_pay.png"
              alt=""
            > {{list.decorateStartTime|getTime}} - {{list.decorateEndTime|getTime}} <span>{{list.amount.toFixed(2)}}元</span></p>
...
<script>
import { payList } from "../../config/decManApi";
import qs from "qs";
export default {
  data() {
    return {
  ...
  methods: {
    getData() {
      ...
      payList(qs.stringify(obj)).then(res => {
        if (res.code === 0) {
